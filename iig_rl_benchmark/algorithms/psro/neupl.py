...
class NeuplSolver(abstract_meta_trainer.AbstractMetaTrainer):
    """
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._total_num_policies = total_num_policies
    ...
    def _initialize_policy(self, initial_policies):
        if self.symmetric_game:
            assert False, "Symmetric game not supported for neupl"
            self._policies = [[]]
            ...
            # self._policies = [
            #     [self._make_policy(player, i) for i in range(self._total_num_policies)]
            #     for player in range(self._num_players)
            # ]
            # self._policies = [[] for _ in range(self._num_players)]
            # print("initial_policies", initial_policies)
            self._policies = initial_policies
            # self._policies[0][0] = policy.UniformRandomPolicy(self._game)
            # self._policies[1][0] = policy.UniformRandomPolicy(self._game)
            self._num_active_policies = [1 for _ in range(self._num_players)]
    ...
    def _initialize_game_state(self):
        ...
        # ]
        self._meta_games = np.array([
            np.full(tuple([self._total_num_policies for _ in range(self._num_players)]), np.nan)
    ...
    def update_meta_strategies(self):
        ...
        self._meta_strategy_probabilities = [None for _ in range(self._num_active_policies[0]+1)]
        self._non_marginalized_probabilities = [None for _ in range(self._num_active_policies[0]+1)]
        assert self._num_active_policies[0] == self._num_active_policies[1]
        for i in range(1, self._num_active_policies[0]+1):
            self._num_active_policies[0], self._num_active_policies[1] = i, i
            # print("i", i, "num_active_policies", self._num_active_policies)
            self._meta_strategy_probabilities[i], self._non_marginalized_probabilities[i] = (
                self._meta_strategy_method(solver=self, return_joint=True)
            )
        # self._num_active_policies[0], self._num_active_policies[1] = self._num_active_policies[0]+1, self._num_active_policies[1]+1
    ...
    def get_policies_and_strategies(self, num_active_policies):
        ...
        probabilities_of_playing_policies = self._meta_strategy_probabilities[num_active_policies]
    ...
    def update_agents(self):
        ...
        for current_player in range(self._num_players):
            for i in range(1, self._num_active_policies[current_player]):
                self._num_active_policies[0] = i
                self._num_active_policies[1] = i
                ...
                (sample_strategy, total_policies, probabilities_of_playing_policies) = (
                    self.get_policies_and_strategies(i)
                )
                ...
                for j in range(len(currently_used_policies)):
                    pol = currently_used_policies[j]
                    ind = current_indexes[j]
                    if ind == 0:
                        # the 0th policy is the uniform random policy, so we don't need to train it.
                        continue
                ...
                if self.symmetric_game:
                    ...
                    self._policies = [self._policies[0]]
                    self._num_players = 1
            self._num_active_policies[0] += 1
            self._num_active_policies[1] += 1
        self._num_active_policies = [self._num_active_policies[k]+1 for k in range(self._num_players)]
    ...
    def update_empirical_gamestate(self, seed=None):
        ...
        total_number_policies = self._num_active_policies
        number_older_policies = [0 for _ in range(self._num_players)]
        number_new_policies = self._num_active_policies
        ...
        print("meta_games")
        nnnnnn = 5
        for row in self._meta_games[0][:nnnnnn]:
            print('\t'.join([f"{x:.2f}" if not np.isnan(x) else "nan" for x in row[:nnnnnn]]))
        for i in range(self._num_active_policies[0]):
            for j in range(self._num_active_policies[1]):
                if hasattr(self._policies[0][i], 'is_frozen'):
                    self._policies[0][i].freeze()
                if hasattr(self._policies[1][j], 'is_frozen'):
                    self._policies[1][j].freeze()
                utility_estimates = self.sample_episodes(
                    [self._policies[0][i], self._policies[1][j]], self._sims_per_entry
                )
                for k in range(self._num_players):
                    if np.isnan(self._meta_games[k][tuple([i, j])]):
                        self._meta_games[k][(i, j)] = utility_estimates[k]
                    else:
                        self._meta_games[k][(i, j)] = self._meta_games[k][(i, j)] * 0.86 + utility_estimates[k] * 0.14
    ...
    def get_meta_game(self):
        ...
        return list(self._meta_games[:, :self._num_active_policies[0], :self._num_active_policies[1]])
    ...
    def get_policies(self):
        ...
        policies = [self._policies[k][:self._num_active_policies[k]] for k in range(self._num_players)]
        if self.symmetric_game:
